...
public class AdminServer extends Application {
   ...
   // Node Sync Queues
    ...
    // TODO: need to load values from String[] args
    // Switch to `api_node_N` for local simulation
//    private static String[] hostNames = {"34.220.38.47", "54.70.5.224", "52.39.165.249", "52.25.143.254", "34.215.193.238"} ;
    private static int hostCounter = 0 ;
    private static String[] hostNames = new String[5];
...
//            Sync sync1 = new Sync( hostNames[0], node1_sync_queue ) ;
//            new Thread(sync1).start() ;
//            Sync sync2 = new Sync( hostNames[1], node2_sync_queue ) ;
//            new Thread(sync2).start() ;
//            Sync sync3 = new Sync( hostNames[2], node3_sync_queue ) ;
//            new Thread(sync3).start() ;
//            Sync sync4 = new Sync( hostNames[3], node4_sync_queue ) ;
//            new Thread(sync4).start() ;
//            Sync sync5 = new Sync( hostNames[4], node5_sync_queue ) ;
//            new Thread(sync5).start() ;
    ...
    public int nodeIndex( String id ) {
        ...
        if (name.equals(hostNames[0])) index = 1;
        else if (name.equals(hostNames[1])) index = 2;
        else if (name.equals(hostNames[2])) index = 3;
        else if (name.equals(hostNames[3])) index = 4;
        else if (name.equals(hostNames[4])) index = 5;
        else index = 0;
...
//        }
    ...
    public void registerNode( String id, String name, String admin_port, String api_port ) {
        ...
        nodes.put( id, node ) ;
        hostNames[hostCounter] = name ;
        hostCounter++;
        System.err.println(" Host Names = " + Arrays.toString(hostNames)) ;
        System.err.println(" Host Counter = " + hostCounter) ;
        if (hostCounter==5) prepareSync();
        System.out.println( "Register Node: " + id + " as: " + name ) ;
    }

    private void prepareSync() {
        System.err.println("\tGet into prepare sync") ;
        try {
            Sync sync1 = new Sync( hostNames[0], node1_sync_queue ) ;
            new Thread(sync1).start() ;
            Sync sync2 = new Sync( hostNames[1], node2_sync_queue ) ;
        ...
        System.err.println("\tSync Queue Successfully Started !") ;
    }
