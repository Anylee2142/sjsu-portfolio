...
//        }
    ...
    private static int identify_node(String key) {
	    int keyNum = key.hashCode() % 5 + 1;
	    System.err.println("Identify Node, Key = " + key + ", hashCode = " + keyNum);
	    return keyNum ;
	}

	private static String[] next_two_indice(int my_index) {
        int[] nextTwoIndice = {(my_index) % 5, (my_index + 1) % 5} ;
        String[] next_two_indice = {Integer.toString(nextTwoIndice[0]+1), Integer.toString(nextTwoIndice[1]+1)} ;
        return next_two_indice ;
    }

    public static JsonRepresentation hash_admin_create_document(String key, String value) throws DocumentException, IOException {
//        String url = String.format("http://api_node_%s:9090/hashing/%s", identified_node, key) ;
...
//        try { jo.put("message", valueJSON.get("message")); }
//        catch (JSONException ex) { System.err.println("Error occurred during structuring Request body !") ; }
//        System.err.println("\t\tJSON OBJECT = " + jo.toString());

//        cr.post(new JsonRepresentation(jo), MediaType.APPLICATION_JSON);
        int identified_node = identify_node(key) ;
        AdminServer.syncDocumentIdentified(key, value, "create", identified_node);
        JSONObject finalResponse = new JSONObject();
        finalResponse.put("request", "create");
        finalResponse.put("key", key);
        finalResponse.put("json", value);
        finalResponse.put("Identified Node", identified_node);

        return new JsonRepresentation(finalResponse);
    }

    public static JsonRepresentation hash_admin_update_document(String key, String value) throws DocumentException  {
        int identified_node = identify_node(key) ;
        AdminServer.syncDocumentIdentified(key, value, "update", identified_node);
        JSONObject finalResponse = new JSONObject();
        System.err.println("\t\t HASH ADMIN UPDATE NEW VALUE = " + value ) ;
        finalResponse.put("request", "update");
        finalResponse.put("key", key);
        finalResponse.put("json", value);
        finalResponse.put("Identified Node", identified_node);
    ...
    public static JsonRepresentation hash_admin_delete_document(String key) throws DocumentException  {
        int identified_node = identify_node(key) ;
        AdminServer.syncDocumentIdentified(key, "", "delete", identified_node);
        JSONObject finalResponse = new JSONObject();
        finalResponse.put("request", "delete");
        finalResponse.put("key", key);
        finalResponse.put("json", "");
        finalResponse.put("Identified Node", identified_node);

        return new JsonRepresentation(finalResponse);
    }

    public static int hash_app_create_document(String key, String value) throws DocumentException {
        // Create document inside
        // And send only sync request only to next two nodes
        ...
        if (doc == null) {
            try {
                ...
                KEYMAP_CACHE.put(key, doc);
//                CREATE_QUEUE.put( doc ) ;
                AdminServer.syncDocumentNextTwo(key, "create", next_two_indice(my_index));
                server.addKeys(key);
                System.out.println("New Document Queued: " + key);
                System.err.println("Document Created ! : " + my_host + ", vclock = " + Arrays.toString(doc.vclock));
                return my_index ;
...
//                CREATE_QUEUE.put( doc ) ;
    ...
    public static void hash_app_update_document(String key, String value) throws DocumentException {
        // vector clocks should be updated locally here
        System.out.println( "Get Document: " + key ) ;
        Document doc = KEYMAP_CACHE.get( key ) ;
        System.err.println("UPDATE DOC = "+ doc);
        ...
        SM.OID record_id = db.getOID( record_key.getBytes() ) ;
        String jsonText = value ;
        System.err.println("@@HASHING APP UPDATE JSONTEXT = " + jsonText) ;
        int size = jsonText.getBytes().length ;
        System.err.println("\tUpdated Document hit here !");
        try {
            // store json to db
            ...
            server.addKeys(key);
            // sync nodes
            AdminServer.syncDocumentNextTwo(key, "update", next_two_indice(my_index));
            return ;
        } catch (SM.NotFoundException nfe) {
            throw new DocumentException( "Document Not Found: " + key ) ;
        } catch (Exception e) {
    ...
    public static void hash_app_delete_document(String key) throws DocumentException {
        System.out.println( "Delete Document: " + key ) ;
        AdminServer server = AdminServer.getInstance() ;
        Document doc = KEYMAP_CACHE.get( key ) ;
        System.err.println("DELETE DOC = "+ doc);
...
//            KEYMAP_CACHE.remove( key ) ;
            ...
            AdminServer.syncDocumentNextTwo(key, "delete", next_two_indice(my_index));
            System.out.println( "Document Deleted: " + key ) ;
            System.err.println( "!!!Document Deleted: " + key ) ;
        }
//        catch (SM.NotFoundException nfe) {
