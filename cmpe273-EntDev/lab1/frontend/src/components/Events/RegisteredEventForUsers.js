import React, { Component } from 'react';
import './RegisteredEventForUsers.css';
import axios from 'axios';
import { Link, NavLink } from 'react-router-dom';
import cookie from 'react-cookies';
import { Redirect } from 'react-router';

import { connect } from 'react-redux';
import * as actionTypes from '../../store/actions';


//create the Navbar Component
class RegisteredEventForUsers extends Component {
    //call the constructor method
    constructor(props) {
        //Call the constrictor of Super class i.e The Component
        super(props);
        //maintain the state required for this component
        this.state = {
            events: [],
            user_pk: this.props.location.state.user_pk,
            errorMessage: ""
        }
    }

    componentWillMount() {
        // Connect DB to fetch events
        console.log("Registered Events local state = ", this.state);

        axios.defaults.withCredentials = true;
        axios.get(`http://localhost:3001/eventRegisters/${this.state.user_pk}`)
            .then(response => {
                console.log("Fetched Events = ", response.data);
                this.setState({
                    events: response.data
                });
                console.log("Chaning Local State Finished !");
            }).catch(error => {
                console.log("Error has been catched : ", error.response.status);
                console.log(error.response);
                console.log("Error response data = ", error.response.data);
                if (true) {
                    this.setState({
                        errorMessage: error.response.data
                    })
                }
            });
    }

    render() {

        document.title = "Find your Events !"
        return (
            <div>
                <div class="events-container">
                    <h1 class="inline-h1">Your Registered Events !</h1>
                    <br></br>
                    <ul>
                        {this.state.events.map(event => (
                            <li>
                                <div class="res_name">
                                    <Link to= {{
                                        pathname: "/eventDetailsUsers",
                                        state: { event: event }
                                    }}>{event.name}</Link>
                                </div>
                                <div>by {event.res_name}</div>
                                <div>
                                    {event.start_time.replace("T", " ").split(".")[0]} ~ {event.end_time.replace("T", " ").split(".")[0]}
                                </div>
                                <div>{event.location}</div>
                                <div>Hash tags: [ {event.hashtag} ]</div>
                            </li>
                        ))}
                    </ul>
                </div>
            </div>
        )
    }
}

export default RegisteredEventForUsers;