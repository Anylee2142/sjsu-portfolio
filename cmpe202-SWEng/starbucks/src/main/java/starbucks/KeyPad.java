...
/** Key Pad */
public class KeyPad implements ITouchEventHandler, IDisplayComponent, IKeyPadSubject, IKeyPadRouterObserver
{
    ...
    /**
     * Observer of KeyPadRouter's Focus Events
     * @param isFocused Is this observer focused?
     */
    public void focusUpdate(boolean isFocused) {
        if (this.isFocused != null) this.isFocused = isFocused ;
        System.out.println(this + "'s isFocused: " + isFocused);
    }
    ...
    public void touch(int x, int y) { 
        if (isValidCoordinate(x, y))
        {
            ...
            if ( x==3 && y==8   )
            {
                if (countPinDigits>0) {countPinDigits--;}
                else {countPinDigits = 0;}
            }
            else if ( y < 8 || (x==2 && y==8))
            {
                countPinDigits++ ;
    ...
    private String getKey( int x, int y )
    {
        ...
        kx = x;
        ky = y-4 ;
        if ( kx==3 && ky ==4 ) return "X" ;
        if ( kx==2 && ky == 4 ) return "0" ;
        if ( kx==1 && ky ==4 ) return " " ;
    ...
    */
    ...
    public void flushCountPinDigits() { this.countPinDigits = 0;}
